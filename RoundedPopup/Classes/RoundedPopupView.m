...
-(id)initWithFrame:(CGRect)frame {
	...
	if ((self = [super initWithFrame:frame])) {
		...
		[button setFrame:CGRectMake(10, 10, buttonImage.size.width, buttonImage.size.height)];
		[button setImage:buttonImage forState:UIControlStateNormal];
		[button addTarget:self action:@selector(buttonPressed:) forControlEvents:UIControlEventTouchUpInside];
		[self addSubview:button];
			
		// Label
		UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(10, buttonImage.size.height + 30, self.bounds.size.width - 10, 20)];
		[label setText:@"Some popup text"];
